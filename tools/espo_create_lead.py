...
@doc_tag("Leads")
@doc_name("Create Lead")
def espo_create_lead_tool(
    ...
    This tool creates a Lead record using common Lead fields. Parameters map
    to EspoCRM Lead attributes; any parameter set to `None` is omitted from
    the request to avoid overwriting defaults.
    ...
    """
    logger.info(f"Request received to create lead with params: {locals()}")
    ...
    params = build_espo_params(locals(), exclude={"duplicate_source_id", "skip_duplicate_check","auth_response"})
    ...
    # Call the instance method which returns a structured dict
    result = client.call_api('POST', 'Lead', params=params, extra_headers=extra_headers if extra_headers else None)
    logger.debug(f"EspoCRM create lead result: {result}")
    return result
