...
def espo_create_contact_tool(
    ...
    account_id: Annotated[Optional[str], Field(description="ID of related Account")] = None,
    accounts_ids: Annotated[Optional[List[str]], Field(description="IDs of related Accounts")] = None,
    account_role: Annotated[Optional[str], Field(description="Account role")] = None,
    opportunity_role: Annotated[Optional[str], Field(description="Opportunity role")] = None,
    ...
    duplicate_source_id: Annotated[Optional[str], Field(description="Record ID of entity being duplicated. Sent as header 'X-Duplicate-Source-Id'.")] = None,
    skip_duplicate_check: Annotated[Optional[bool], Field(description="Skip duplicate check. Sent as header 'X-Skip-Duplicate-Check'.")] = None,
    ...
    Example Requests:
    - Create a basic contact: espo_create_contact_tool(first_name="Jane", last_name="Doe", email_address="jane@example.com")
    - Create a contact with multiple emails and phones: espo_create_contact_tool(first_name="Acme", last_name="Inc", email_address_data=[{"emailAddress": "a@acme.com", "primary": True}], phone_number_data=[{"phoneNumber": "+123456789", "primary": True}])
    - Create a contact with custom fields: espo_create_contact_tool(first_name="Demo", last_name="User", custom_fields={"c_demo_field": "value"})
    ...
    result = client.call_api('POST', 'Contact', params=params, extra_headers=extra_headers if extra_headers else None)
