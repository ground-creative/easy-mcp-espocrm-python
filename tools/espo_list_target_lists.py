...
def espo_list_target_lists_tool(
    ...
    - Fetch first 50 TargetLists with selected attributes:
      `espo_list_target_lists_tool(attribute_select=["name", "assignedUserName"], max_size=50)`
    - Fetch only TargetLists assigned to current user:
      `espo_list_target_lists_tool(bool_filter_list=["onlyMy"])`
    - Fetch TargetLists using advanced whereGroup filter:
      `espo_list_target_lists_tool(where_group=[{"type": "equals", "attribute": "targetStatus", "value": "Listed"}])`
