...
def espo_create_target_list_tool(
    name: Annotated[
    ...
    `espo_create_target_list_tool(name="Prospects 2026")`
    - Create a TargetList with a category and assigned user:
    `espo_create_target_list_tool(name="Campaign Leads", category_id="cat123", assigned_user_id="user456")`
    - Create a TargetList with custom fields:
    `espo_create_target_list_tool(name="Demo List", custom_fields={"c_priority": "High"})`
