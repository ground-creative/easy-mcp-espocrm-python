...
def espo_update_campaign_tool(
    campaign_id: str,
    name: Annotated[Optional[str], Field(description="Campaign name (<=255 chars)")] = None,
    ...
    budget_currency: Annotated[Optional[str], Field(description="Budget currency code (USD, EUR)")] = None,
    ...
    - Update campaign name and status:
      espo_update_campaign_tool(campaign_id="abc123", name="Spring Promo", status="Active")
    - Update campaign teams and budget:
      espo_update_campaign_tool(campaign_id="abc123", teams_ids=["team1","team2"], budget=10000, budget_currency="USD")
    - Update custom fields on a campaign:
      espo_update_campaign_tool(campaign_id="abc123", custom_fields={"c_campaign_code":"X123","c_priority":2})

    Returns:
    - A structured dict containing the API response with keys:
      `status_code`, `ok`, `data`, `error`, and `error_type`.
    """
    logger.info(f"Request received to update campaign {campaign_id} with params: {locals()}")
