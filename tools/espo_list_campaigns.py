...
def espo_list_campaigns_tool(
    ...
    primary_filter: Annotated[Optional[str], Field(description="Primary filter: active")] = None,
    ...
    espo_list_campaigns_tool(attribute_select=["name", "status"], max_size=50)
    - Fetch campaigns assigned only to the current user, sorted by creation date descending:
    espo_list_campaigns_tool(bool_filter_list=["onlyMy"], order="desc", order_by="createdAt")
    - Fetch campaigns using advanced whereGroup filter:
    espo_list_campaigns_tool(where_group=[{"type": "equals", "attribute": "status", "value": "Active"}, {"type": "contains", "attribute": "name", "value": "Launch"}])
    - Fetch campaigns with a primary filter:
    espo_list_campaigns_tool(primary_filter="completed", max_size=20)
    - Fetch campaigns using a text search query:
    espo_list_campaigns_tool(text_filter="Summer*2026", max_size=10)
