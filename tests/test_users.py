...
from app.tools.espo_get_user import espo_get_user_tool

# from app.tools.espo_update_user import espo_update_user_tool

sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..")))


def test_espo_list_users_tool(api_key_setup):
    ...
    result = espo_list_users_tool(max_size=2)
...
def test_espo_search_leads_tool(api_key_setup):
    ...
    search_term = "admin"
    result = espo_list_users_tool(text_filter=search_term)
...
def test_espo_get_user_tool(api_key_setup):
    ...
    get_result = espo_get_user_tool(user_id=fixture_id)
...
# def test_espo_update_user_tool(api_key_setup):
...
#    res = espo_update_user_tool(user_id=fixture_id, title=new_title)
#    assert isinstance(res, dict)
#    assert "status_code" in res and res["status_code"] == 200
#    assert "ok" in res and res["ok"] is True

#    update = espo_update_user_tool(user_id=fixture_id, title=old_title)
